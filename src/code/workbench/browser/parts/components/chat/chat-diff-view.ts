...
export function ChatDiffView(opts: {
  ...
  const settle = (label: string) => {
    btn_row.innerHTML = "";
    const done = h("span", {
      class: "text-[10px] text-chat-foreground/50 select-none",
    });
    done.textContent = label;
    btn_row.appendChild(done);
  };

  if (opts.onAccept || opts.onReject) {
    if (opts.onReject) {
      const btn = h("button", {
        class:
    ...
    if (opts.onAccept) {
      const btn = h("button", {
        class:
          "h-5 px-2 text-[10px] rounded-[4px] bg-transparent border-0 cursor-pointer transition-colors font-medium",
        attrs: { type: "button" },
      });
